...
export default function StudentFaceCapture() {
    ...
    if (imageSrc) {
      ...
      try {
        ...
        if (result.success) {
          ...
          setUserName(result.user_name || prn)
  ...
  const updateProfileImage = (imageData: string) => {
    try {
      ...
      // Update student users database
      const studentUsers = JSON.parse(localStorage.getItem('student_users') || '[]')
      const userIndex = studentUsers.findIndex((user: any) => user.student_id === prn || user.email === prn)
      if (userIndex !== -1) {
        studentUsers[userIndex].profile_image = imageData
        localStorage.setItem('student_users', JSON.stringify(studentUsers))
      }

      console.log('Profile image updated automatically after face capture')
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <div className="w-full max-w-md bg-white p-6 rounded-lg shadow">
        <h1 className="text-xl font-bold mb-6 text-center">Student Face Capture</h1>
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col items-center space-y-4">
            ...
            {capturedImage && (
              <div className="text-center space-y-2">
                <h2 className="text-xl font-semibold text-gray-800">{userName || prn}</h2>
          ...
          {!capturedImage ? (
            <div className="space-y-4 px-2">
              ...
              <button
                ...
                disabled={!prn.trim() || !faceDetected || isProcessing}
                className={`w-full py-3 px-4 rounded-lg font-medium transition-colors ${
                  faceDetected && prn.trim() && !isProcessing
