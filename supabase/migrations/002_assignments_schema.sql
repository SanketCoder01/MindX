...
-- Create RLS policies for faculty
...
-- Create RLS policies for assignment resources
CREATE POLICY "Faculty can manage resources for their assignments" ON assignment_resources
    FOR ALL USING (
        EXISTS (
...
CREATE POLICY "Students can view resources for accessible assignments" ON assignment_resources
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM assignments 
            JOIN students ON students.department = assignments.department 
                AND students.year = assignments.year
            WHERE assignments.id = assignment_resources.assignment_id 
...
CREATE POLICY "Faculty can update grades for their assignments" ON assignment_submissions
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM assignments 
            WHERE assignments.id = assignment_submissions.assignment_id 
...
CREATE POLICY "Faculty can upload assignment resources" ON storage.objects
    FOR INSERT WITH CHECK (
        bucket_id = 'assignment-resources' 
        AND auth.role() = 'authenticated'
    );

CREATE POLICY "Anyone can view assignment resources" ON storage.objects
    FOR SELECT USING (bucket_id = 'assignment-resources');
...
CREATE POLICY "Faculty can view submissions for their assignments" ON storage.objects
    FOR SELECT USING (
        bucket_id = 'assignment-submissions' 
        AND EXISTS (
            SELECT 1 FROM assignments 
