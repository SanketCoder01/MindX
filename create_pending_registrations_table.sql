-- Run this SQL in your Supabase SQL Editor to create the pending_registrations table
...
-- Policy: Faculty can view all pending registrations (simplified admin check)
CREATE POLICY "Faculty can view all pending registrations" ON public.pending_registrations
    FOR SELECT USING (
        EXISTS (
...
-- Policy: Faculty can update all pending registrations (for approval/rejection)
CREATE POLICY "Faculty can update all pending registrations" ON public.pending_registrations
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM public.faculty 
            WHERE user_id = auth.uid()
